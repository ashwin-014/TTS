    ...
    def tts(
        ...
        if not reference_wav:
            ...
            if not use_gl:
                ...
                print(f"Vocoder total elapsed time: {time.perf_counter() - st}")

                ast = time.perf_counter()
                # pad of 5 mel frames before and after
                # hops * mel frames = length of audio
                # attn = torch.sum(outputs["outputs"]["alignments"], dim=(-1, -2))
                attn = outputs["outputs"]["alignments"].cpu().numpy()
                attn = attn.sum(axis=(-1, -2))
                print(f"Attn torch sum total elapsed time: {time.perf_counter() - st}")
                st = time.perf_counter()
                # TODO: get a permanent solution ta a mask level to silence 5 mel frames
                attn = attn - 5
                print(f"Attn 5 mel subtraction elapsed time: {time.perf_counter() - st}")
                st = time.perf_counter()
                # l = int(waveform.shape[2] / attn.max(dim=0))
                l = int(waveform.shape[2] / attn.max(axis=0))
                # reduce(np.maximum, matrix)
                print(f"Attn multiplication elapsed time: {time.perf_counter() - st}")
                attn = attn * l
                print(f"Attn multiplication elapsed time: {time.perf_counter() - st}")
                # attn = attn.to(torch.int)
                attn = attn.astype(np.int)
                print(f"Attn multiplication elapsed time: {time.perf_counter() - st}")
                print(f"Attn modification total elapsed time: {time.perf_counter() - ast}")
            ...
            print(f"Waveform to cpu and numpy total elapsed time: {time.perf_counter() - st}")
            ...
            for i, wave in enumerate(waveform):
                ...
                wavs += list(wave)
                wavs += [0] * 10000
            print(f"WAV conversion total elapsed time: {time.perf_counter() - st}")
        else:
