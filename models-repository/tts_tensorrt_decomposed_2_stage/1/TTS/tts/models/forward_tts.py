...
class ForwardTTS(BaseTTS):
    ...
    def _forward_decoder(
        ...
        x_mask_ex = torch.unsqueeze(x_mask, 1)
        # y_mask = torch.unsqueeze(sequence_mask(y_lengths, torch.max(y_lengths)), 1).to(o_en.dtype)
        y_mask = torch.unsqueeze(sequence_mask(y_lengths, None), 1).to(o_en.dtype)
        o_en_ex_in, attn = self.expand_encoder_outputs(o_en, dr, x_mask_ex, y_mask)
        # positional encoding
        if hasattr(self, "pos_encoder"):
            o_en_ex_out = self.pos_encoder(o_en_ex_in, y_mask)
        # decoder pass
        o_de = self.decoder(o_en_ex_out, y_mask, g=g)
    ...
    def forward(
        ...
        # y_mask = torch.unsqueeze(sequence_mask(y_lengths, torch.max(y_lengths)), 1).float()
        # encoder pass
