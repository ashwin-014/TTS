...
def trt_exec(
    ...
    err, = cuda.cuMemcpyHtoD(
        ...
        trt.volume((bs_to_use, inputs.shape[1])) * np.dtype(trt.nptype(fastpitch["engine"].get_binding_dtype("x"))).itemsize
    ...
    fastpitch["context"].set_binding_shape(fastpitch["engine"].get_binding_index("x"), inputs.shape)
    print(fastpitch["engine"].get_tensor_shape("x"))
    
    # fastpitch["io"]["inputs"]["speaker_id"][:bs_to_use] = speaker_id[:bs_to_use]
    ...
    # fastpitch["context"].set_binding_shape(fastpitch["engine"].get_binding_index("speaker_id"), (1))#speaker_id.shape)
    ...
    model_outputs_binding_shape = fastpitch["engine"].get_binding_shape("model_outputs")
    print(model_outputs_binding_shape[0], model_outputs_binding_shape[1], model_outputs_binding_shape[2])
    err, = cuda.cuMemcpyDtoH(
        fastpitch["io"]["outputs"]["model_outputs"].ctypes.data,
        fastpitch["io"]["device_mem_address"]["model_outputs"],
        trt.volume((bs_to_use, model_outputs_binding_shape[1], model_outputs_binding_shape[2])) * np.dtype(trt.nptype(fastpitch["engine"].get_binding_dtype("model_outputs"))).itemsize,
        # fastpitch["data_stream"]
    )

    alignments_binding_shape = fastpitch["engine"].get_binding_shape("model_outputs")
    err, = cuda.cuMemcpyDtoH(
        fastpitch["io"]["outputs"]["alignments"].ctypes.data,
        fastpitch["io"]["device_mem_address"]["alignments"],
        trt.volume((bs_to_use, alignments_binding_shape[1], alignments_binding_shape[2])) * np.dtype(trt.nptype(fastpitch["engine"].get_binding_dtype("alignments"))).itemsize,
        # fastpitch["data_stream"]
    )
    ...
    err, = cuda.cuMemcpyHtoD(
        ...
        trt.volume((bs_to_use, 80, 817)) * np.dtype(trt.nptype(hifigan["engine"].get_binding_dtype("c"))).itemsize
        # hifigan["data_stream"]
    )
    hifigan["context"].set_binding_shape(hifigan["engine"].get_binding_index("c"), vocoder_inputs[:bs_to_use].shape)
    assert hifigan["context"].all_binding_shapes_specified
    hifigan["context"].execute_v2(bindings=hifigan["io"]["bindings"])

    o_binding_shape = hifigan["engine"].get_binding_shape("o")
    err, = cuda.cuMemcpyDtoH(
        hifigan["io"]["outputs"]["o"].ctypes.data,
        hifigan["io"]["device_mem_address"]["o"],
        # trt.volume((bs_to_use, o_binding_shape[1], o_binding_shape[2])) * np.dtype(trt.nptype(hifigan["engine"].get_binding_dtype("o"))).itemsize,
...
def trt_batch():
    ...
    wav = np.array(wav)
    save_wav(wav=wav, path="nosqueeze/trt_output_new.wav", sample_rate=22050)
