...
class ForwardTTS(BaseTTS):
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None}):  # pylint: disable=unused-argument
        ...
        # ----------
        print(f"Input batch total elapsed time: {time.perf_counter() - st}")
        ...
        print(f"Fastpitch Encoder total elapsed time: {time.perf_counter() - st}")
        # pitch predictor pass
        o_pitch = None
        if self.args.use_pitch:
            st = time.perf_counter()
            ...
            o_en = o_en + o_pitch_emb
            print(f"Fastpitch Pitch predictor total elapsed time: {time.perf_counter() - st}")
        # energy predictor pass
        o_energy = None
        if self.args.use_energy:
        ...
        print(f"Fastpitch Decoder total elapsed time: {time.perf_counter() - st}")
        outputs = {
            "model_outputs": o_de,
            "alignments": attn,
            "pitch": o_pitch,
